...
Lastly we have the binary constraints:
...
Additionally we want to produce a certain Product only if it is profitable after deducting the fixed cost so we will have:

\begin{align*}
A &\geq 600/2.14\cdot Y_A = 280.24 Y_A\\
B &\geq 500/1.13\cdot Y_B = 444.44 Y_B\\
C &\geq 250/4\cdot Y_C = 62.5 Y_C\\
D &\geq 300/2.12\cdot Y_D = 141.78 Y_D
\end{align*}
...
bin_model = make.lp(0,8)                            # 8 Variables (A,B,C,D)
...
add.constraint(bin_model,
               ...
               type = "<=",
               rhs = 0,
               indices = c(1,5)) #binary contraint for A

add.constraint(bin_model,
               xt = c(1,-1636.36),
               type = "<=",
               rhs = 0,
               indices = c(2,6)) #binary contraint for B

add.constraint(bin_model,
               xt = c(1,-1200),
               type = "<=",
               rhs = 0,
               indices = c(3,7)) #binary contraint for C

add.constraint(bin_model,
               xt = c(1,-1300),
               type = "<=",
               rhs = 0,
               indices = c(4,8)) #binary contraint for D
...
#Setting lower and upper bounds
set.bounds(bin_model,lower = c(1200,450,0,0,0,0,0,0), upper = c(Inf,650,120,150,1,1,1,1)) 
