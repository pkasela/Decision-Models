...
gradientDescent <- function(par.init, fun.gr, lr, iters){
  ...
  for(k in 1:iters){
    loss[k,]  <- fn.gr(par[k,])
    lr = lr*exp(-k*lr^3/iters) #can be a solution
...
ris_temp <- lapply(1:length(y), function(x) {
  gradientDescent(random.points[x,],fun.gr,0.1,1000)})
...
plot(1:length(ris_temp[[which.min(distances)]]$dist),
     ris_temp[[which.min(distances)]]$dist,type="l",
...
ansfgh <- optimr(par = strt, fn = fn.distance, gr = fn.gr,  method="CG")
proptimr(ansfgh) 
```
...
plot(1:2001,ris,type = "l",xlab = "Iterations", ylab = "Distance")
```
